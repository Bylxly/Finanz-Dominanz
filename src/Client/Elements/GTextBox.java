...
public class GTextBox {
    ...
    private int backgroundColor, textColor, borderColor;
    private boolean isActive;
    private boolean focused = false;

    public GTextBox(float x, float y, float width, float height, int backgroundColor, int textColor, int borderColor, boolean isActive) {
        this.x = x;
        this.y = y;
        this.width = width;
        this.height = height;
    ...
    public void keyPressed(char key, int keyCode) {
        if (focused) {
            if (keyCode == PApplet.BACKSPACE && text.length() > 0) {
                ...
                focused = false; // Exit focus when Enter is pressed
                saveText(); // Save the text when Enter is pressed
